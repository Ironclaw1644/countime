...
export function MapFilters({ filters, onChange, allStates }: Props) {
  ...
  const activeFilterCount = useMemo(() => {
    ...
    if (filters.selfSurrenderOnly) n++;
    if (filters.gender !== 'ALL') n++;
    if (!filters.showHolding) n++;
    if (filters.showClosed) n++;
    if (filters.userZip) n++;
    return n;
  }, [filters]);

  return (
    <div className="pointer-events-auto absolute left-4 top-4 z-20 w-[min(calc(100%-2rem),22rem)] rounded border border-rule bg-paper-raised/92 shadow-raise backdrop-blur">
      {/* Search row */}
      <div className="flex items-center gap-2 px-4 pt-3.5">
        ...
        {queryDraft && (
          <button
            ...
            className="rounded-full p-1 text-ink-muted hover:bg-paper-sunk hover:text-ink"
          >
            <Icon icon={faXmark} />
          </button>
        )}
      ...
      <div className="flex items-center gap-2 px-3 pb-3 pt-2">
        <div className="flex flex-1 items-center gap-2 rounded-full border border-rule bg-paper px-3 py-1.5">
          <Icon icon={faLocationCrosshairs} className="text-accent text-[12px]" />
          <input
            type="text"
            inputMode="numeric"
            maxLength={5}
        ...
        <button
          ...
          className={cn(
            'relative grid h-9 w-9 place-items-center rounded-full border border-rule',
            expanded ? 'bg-accent text-accent-on' : 'bg-paper text-ink hover:bg-paper-sunk',
          )}
        >
          <Icon icon={faSliders} />
          {activeFilterCount > 0 && (
            <span className="absolute -right-1 -top-1 grid h-4 min-w-[16px] place-items-center rounded-full bg-accent px-1 text-[10px] font-semibold text-accent-on">
              {activeFilterCount}
            </span>
          )}
        </button>
      ...
      {zipError && (
        <p className="px-4 pb-3 text-[11px] text-accent-hover">{zipError}</p>
      )}
      ...
      {expanded && (
        <div className="space-y-4 border-t border-rule px-4 py-4">
          <div>
            <p className="eyebrow text-[10px] text-ink-muted">Who&rsquo;s housed</p>
            <div className="mt-2 inline-flex rounded-full border border-ink/15 bg-paper p-0.5">
              <SegmentButton
          ...
          <div>
            <p className="eyebrow text-[10px] text-ink-muted">Programs &amp; eligibility</p>
            <div className="mt-2 flex flex-wrap gap-2">
              <ToggleChip
                active={filters.selfSurrenderOnly}
                onToggle={() => onChange({ selfSurrenderOnly: !filters.selfSurrenderOnly })}
                icon={faRightToBracket}
                label="Self-surrender"
              />
              <ToggleChip
              ...
              <ToggleChip
                ...
                onToggle={() => onChange({ showClosed: !filters.showClosed })}
          ...
          <div>
            <p className="eyebrow text-[10px] text-ink-muted">State</p>
                  ...
                  <button
                    ...
                    className={cn(
                      ...
                      active
                        ...
                        : 'border-ink/15 bg-paper text-ink-soft hover:bg-paper-sunk',
                    )}
                  >
                    {st}
                  </button>
